...
export default function AdminPage() {
  ...
  const [onglet, setOnglet] = useState<"rsvps" | "messages" | "musiques">("rsvps");

  const confirmes = rsvps.filter((r) => r.statut === "confirme");
  const declines = rsvps.filter((r) => r.statut === "decline");

  const handleLogin = () => {
    if (motDePasse === MOT_DE_PASSE) { setConnecte(true); setErreur(false); }
    else setErreur(true);
  };
    ...
    return (
      <main style={{ minHeight: "100vh", background: "#faf8f5", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", ...garamond }}>
        <div style={{ maxWidth: "360px", width: "100%", textAlign: "center" }}>
          ...
          <p style={{ letterSpacing: "0.3em", fontSize: "10px", textTransform: "uppercase", color: "#b89a6a", marginBottom: "0.5rem" }}>Espace privé</p>
          <h1 style={{ fontSize: "2rem", fontWeight: 300, color: "#2c2118", marginBottom: "2rem" }}>Administration</h1>
          <input
            type="password"
            ...
            style={{ width: "100%", border: "1px solid #d8cfc4", background: "transparent", padding: "12px 20px", fontSize: "1rem", color: "#2c2118", outline: "none", marginBottom: "1rem", boxSizing: "border-box", ...garamond }}
          />
          {erreur && <p style={{ color: "#c0392b", fontSize: "0.85rem", marginBottom: "1rem", fontStyle: "italic" }}>Mot de passe incorrect.</p>}
          <button onClick={handleLogin} style={{ width: "100%", padding: "12px", background: "#b89a6a", color: "white", border: "none", fontSize: "11px", letterSpacing: "0.25em", textTransform: "uppercase", cursor: "pointer", ...garamond }}>
            Accéder
          </button>
        </div>
      </main>
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#faf8f5", ...garamond }}>
      ...
      <div style={{ background: "#2c2118", padding: "1.5rem 2rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => setConnecte(false)} style={{ background: "transparent", border: "1px solid #b89a6a", color: "#b89a6a", padding: "6px 16px", fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer", ...garamond }}>
          Déconnexion
        </button>
      </div>

      {loading ? (
        <div style={{ textAlign: "center", padding: "4rem", color: "#b89a6a", letterSpacing: "0.2em" }}>Chargement…</div>
      ) : (
        <div style={{ maxWidth: "900px", margin: "0 auto", padding: "2rem 1rem" }}>
          ...
          {/* Onglets */}
          <div style={{ display: "flex", marginBottom: "1.5rem", borderBottom: "1px solid #e8e0d5" }}>
            {([
              ...
              <button key={key} onClick={() => setOnglet(key)} style={{
                ...
                borderBottom: onglet === key ? "2px solid #b89a6a" : "2px solid transparent",
                color: onglet === key ? "#b89a6a" : "#a89880",
                    ...
                    {rsvps.map((r, i) => (
                      <tr key={r.id} style={{ borderTop: "1px solid #f0ebe4", background: i % 2 === 0 ? "white" : "#faf8f5" }}>
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <span style={{ display: "inline-block", padding: "3px 12px", background: r.statut === "confirme" ? "#f0f7f0" : "#fdf0f0", color: r.statut === "confirme" ? "#4a7c59" : "#c0392b", fontSize: "10px", letterSpacing: "0.15em", textTransform: "uppercase" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {messages.length === 0 ? (
                ...
                <div key={msg.id} style={{ background: "white", border: "1px solid #e8e0d5", padding: "1.5rem", borderLeft: "3px solid #b89a6a" }}>
                  <p style={{ fontSize: "1.1rem", fontWeight: 300, color: "#2c2118", lineHeight: 1.7, marginBottom: "0.75rem", fontStyle: "italic" }}>"{msg.message}"</p>
          ...
          {onglet === "musiques" && (
            <div style={{ background: "white", border: "1px solid #e8e0d5", overflow: "hidden" }}>
              {musiques.length === 0 ? (
                <p style={{ textAlign: "center", padding: "3rem", color: "#a89880", fontStyle: "italic" }}>Aucune suggestion pour l'instant.</p>
              ) : (
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#f3ede4" }}>
                      {["Nom", "Suggestion musicale", "Date"].map((h) => (
                        <th key={h} style={{ padding: "12px 16px", textAlign: "left", fontSize: "10px", letterSpacing: "0.2em", textTransform: "uppercase", color: "#b89a6a", fontWeight: 400 }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {musiques.map((m, i) => (
                      <tr key={m.id} style={{ borderTop: "1px solid #f0ebe4", background: i % 2 === 0 ? "white" : "#faf8f5" }}>
                        <td style={{ padding: "12px 16px", color: "#2c2118", fontSize: "1rem", fontWeight: 300 }}>{m.nom}</td>
                        <td style={{ padding: "12px 16px", color: "#4a3f35", fontSize: "1rem", fontWeight: 300 }}>🎵 {m.musique}</td>
                        <td style={{ padding: "12px 16px", color: "#a89880", fontSize: "0.85rem" }}>{formatDate(m.createdAt)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
